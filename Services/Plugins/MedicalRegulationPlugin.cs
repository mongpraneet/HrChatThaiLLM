...
public class MedicalRegulationPlugin
{
    ...
    public MedicalRegulationPlugin(IWebHostEnvironment env, IConfiguration config)
    {
        _env = env;
        _config = config;
        PreloadRegulation();
    ...
    private void PreloadRegulation()
    {
        ...
        lock (_lock)
        {
            ...
            try
            {
                var relativePath = _config["FilePaths:MedicalRegulationExcel"] ?? "file/Intent/Medical expense.xlsx";
