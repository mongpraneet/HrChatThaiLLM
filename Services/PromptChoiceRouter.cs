...
public class PromptChoiceRouter : IPromptChoiceRouter
{
    ...
    public bool TryHandle(string userId, string message, out string response)
    {
        ...
        if (normalizedNoSpace == "ผม" || normalizedNoSpace == "ผมคือ")
        {
            _contexts[uid] = new SessionState { Context = ChoiceContext.Profile };
            response = """
                คุณหมายถึง โปรไฟล์ของคุณใช่หรือไม่ กรุณาเลือกเมนูด้วยตัวเลข
                1. โปรไฟล์ของคุณ
                2. ยกเลิก (fallback)
                """;
            return true;
...
""";
        ...
        if (_contexts.TryGetValue(uid, out var state))
        {
            ...
            if (state.Context == ChoiceContext.Profile && TryHandleProfileChoice(normalized, out response))
            {
                _contexts.TryRemove(uid, out _);
                return true;
            }
    ...
    private static bool TryHandleGraphChoice(string normalized, string adYear, string dispYear, out string response)
    {
        ...
        if (normalized is "3" or "3." or "๓")
        {
            response = string.IsNullOrEmpty(adYear) 
                ? "[TRAINING_CHART]" 
                : $"[TRAINING_CHART:{adYear}]";
            return true;
        }
        if (normalized is "4" or "4." or "๔")
        {
            response = "[ACTION:FALLBACK]";
            return true;
        }
    ...
    // 🌟 UPGRADE: เมธอดช่วยจัดการคำตอบของเมนูโปรไฟล์ ยิง Token ส่งกลับไปหน้าจอให้ Active ทำงานทันที
    private static bool TryHandleProfileChoice(string normalized, out string response)
    {
        response = string.Empty;
        if (normalized is "1" or "1." or "๑" or "ใช่" or "โปรไฟล์")
        {
            // พ่นรหัสลับออกไปยิง Active ข้อมูลโปรไฟล์ตามที่คุณต้องการ
            response = "[ACTION:SHOW_PROFILE]";
            return true;
        }
        if (normalized is "2" or "2." or "๒" or "ไม่ใช่" or "ยกเลิก")
        {
            response = "[ACTION:FALLBACK]";
            return true;
        }
